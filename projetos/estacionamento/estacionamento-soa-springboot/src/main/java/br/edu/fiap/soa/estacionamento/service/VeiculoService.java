...
// Agora valida a placa para não haver duplicações
    @Transactional
    public Veiculo criar(Veiculo v) {
        if (repo.existsByPlaca(v.getPlaca())) {
            throw new RuntimeException("Placa já cadastrada");
        }

        long totalVeiculos = repo.count();
        int limite = 100; // poderia vir de configuração externa
        if (totalVeiculos >= limite) {
